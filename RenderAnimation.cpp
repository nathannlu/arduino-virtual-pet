...
RenderAnimation::RenderAnimation(Display* disp) : displayPtr(disp) {}

void RenderAnimation::renderStats(int hunger, int play, int rave) {
...
void RenderAnimation::renderPet(AnimationType animType, int x, int y, int direction) {
  Adafruit_PCD8544* disp = displayPtr->getDisplay();
...
void RenderAnimation::playPlayAnimation(int x, int y, int direction) {
  Adafruit_PCD8544* disp = displayPtr->getDisplay();
  Partition* bottom = displayPtr->getBottomPartition();
  Partition* textBox = displayPtr->getTextBoxPartition();
...
void RenderAnimation::playRaveAnimation(int x, int y, int direction) {
  Adafruit_PCD8544* disp = displayPtr->getDisplay();
  Partition* textBox = displayPtr->getTextBoxPartition();
...
void RenderAnimation::playDeathAnimation() {
  Adafruit_PCD8544* disp = displayPtr->getDisplay();
  Partition* bottom = displayPtr->getBottomPartition();
  Partition* textBox = displayPtr->getTextBoxPartition();
...
const unsigned char* RenderAnimation::getSpriteForAnimation(AnimationType type) {
  switch (type) {
    case ANIM_IDLE:
